...
class SLExperiment:
    ...
    def __init__(self, config, other_callbacks=None, progbar=True, project='RobustLyaNet'):
        OmegaConf.resolve(config)
        self.config = config

        logger = pl_loggers.WandbLogger(name=self.create_log_name(),
                                        project=project,
                                        log_model='all')
        ...
        if not config.disable_logs:
            ...
            if config.module.val_adv:
                callbacks += [
                    pl.callbacks.ModelCheckpoint(monitor='validation_adv_error',
                                                 save_top_k=1,
                                                 mode='min')
                ]
            else:
        ...
        self.trainer = pl.Trainer(
            ...
            max_epochs=config.module.max_epochs,
            log_every_n_steps=1,
    ...
    def run(self, checkpoint_module=None, test_only=False):
        ...
        if not test_only:
            self.trainer.fit(module,
                             train_loader,
                             val_loader)
        # # load in the best model, test with single gpu
        # dist.destroy_process_group()
        # if self.trainer.is_global_zero:
        #     self.trainer.strategy.num_nodes = 1
        #     # self.trainer.training_type_plugin.num_nodes = 1
        #     # self.trainer.strategy.num_processes = 1
        # else:
        #     return
        ...
        self.trainer.test(ckpt_path="best", dataloaders=test_loader)

        # dist.destroy_process_group()
        # if self.trainer.is_global_zero:
        #     trainer = pl.Trainer(gpus=1)
        #     trainer.test(ckpt_path=self.trainer.callbacks[2].best_model_path, dataloaders=test_loader)
