# testing pipeline, only use 1 gpu
import hydra
import ExpConfig
...
class SLExperiment:
    ...
    def __init__(self, config, other_callbacks=None, progbar=True, project='RobustLyaNet-Test'):
        ...
        #     save_dir=str(tensorboard_root / self.save_dir), flush_secs=1)
    ...
    def run(self, checkpoint_module=None, test_only=False):
        ...
        if not test_only:
            self.trainer.fit(module,
                             train_loader,
                             val_loader)
        self.trainer.test(module, dataloaders=test_loader)
        return module
